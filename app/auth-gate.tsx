...
import { Leaf, Link, LockKeyhole, LogOut, Mail, ShieldCheck } from 'lucide-react';
...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  async function signInWithGoogle() { setMessage('Opening Google sign-in…'); const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: window.location.origin } }); if (error) setMessage(error.message); }
  ...
  if (!session) return <AuthShell><div className="auth-form"><Link/><h1>Welcome to Arshistudio</h1><p>Sign in securely with your Google account.</p><button type="button" className="primary" onClick={signInWithGoogle}>Continue with Google</button><div className="auth-divider">or use email</div><form onSubmit={sendLink} className="auth-form"><Mail/><label>Email<input value={ALLOWED_EMAIL} readOnly/></label><button className="secondary">Email me a secure sign-in link</button></form>{message && <output>{message}</output>}</div></AuthShell>;
