...
export async function POST(request:Request){
 ...
 try{
  ...
  if(!response.ok)return json({error:response.status===429?'AI free quota is unavailable or exhausted. Use local capture or try later.':response.status===400||response.status===403?'Google rejected the AI key or project permissions. Check the Gemini key and API access in Google AI Studio.':response.status===404?'The configured Gemini model is unavailable for this project.':'AI could not read this document. Use local capture or try later.'},response.status===429?429:502);
